...
## TODO — Version Unification (P3)

> Monorepo 内前端项目存在多代框架版本并存，需在后续迭代中统一。

### 版本分裂现状

| 依赖 | 旧版 | 新版 | 涉及项目 |
|------|------|------|----------|
| **React** | 18.3.1 | 19.x | agv-protocol-app, G3-Funding, template 用 18；其余 4 个用 19 |
| **Next.js** | 14.2.33 | 15.x | G3-Funding, template 用 14；其余 5 个用 15 |
| **Tailwind CSS** | v3 (.config.js) | v4 (@import) | agv-protocol-app, G3-Funding, template 用 v3；其余 4 个用 v4 |
| **Firebase** | 10.14.1 | 12.3.0 | agv-protocol-app, buy-page, investor-portal 用 10；G3-Funding, asset 用 12 |
| **Zod** | 3.x | 4.x | agv-protocol-app, G3-Funding 用 3；asset 用 4 |
| **Thirdweb** | v4 SDK | v5 SDK | G3-Funding **同时依赖** `@thirdweb-dev/sdk@^4` + `thirdweb@^5`（需迁移为纯 v5） |

### 统一目标（建议）

```
React 19 + Next.js 15 + Tailwind v4 + Firebase 12 + Zod 4 + Thirdweb v5
```

### 迁移优先级

1. **P3-1 Thirdweb v4→v5**（G3-Funding）— 去除 `@thirdweb-dev/sdk` v4 双版本共存
2. **P3-2 Next 14→15 + React 18→19**（G3-Funding, template）— 影响 Server Components / Turbopack
3. **P3-3 Tailwind v3→v4**（agv-protocol-app, G3-Funding, template）— 配置从 JS 迁至 CSS @import
4. **P3-4 Firebase 10→12**（agv-protocol-app, buy-page, investor-portal）— API 变更较小
5. **P3-5 Zod 3→4**（agv-protocol-app, G3-Funding）— Schema API breaking changes

### 实施状态摸底（2026-03）

全量审计结果记录在 [AGENTS.md](AGENTS.md#实施状态摸底2026-03-审计) 中，摘要：

- **合约测试**：3 模块 **484** 个测试全绿，**19/19** 合约覆盖；P0-01/P0-02 已修复，pGVT 测试已补全
- **前端构建**：7/7 编译通过，4 个缺 `.env.local`（已创建 `.env.example`），G3-Funding 依赖已修复（npm → pnpm）
