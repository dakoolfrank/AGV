...
# collect 子模块
COLLECT_SCRIPTS_DIR = ROOT_DIR / "modules" / "collect" / "scripts"
sys.path.insert(0, str(COLLECT_SCRIPTS_DIR))

from skill_collect import CollectSkill
...
class TestArbPipelineStructure:
    ...
    def test_steps_ordered(self):
        assert ArbCampaignLoop.STEPS == ["collect", "curate", "dataset", "execute", "fix"]
    ...
    def test_retreat_levels(self):
        ...
        assert RETREAT_LEVELS["C"]["target_step"] == "collect"
        # A 级无 LLM
