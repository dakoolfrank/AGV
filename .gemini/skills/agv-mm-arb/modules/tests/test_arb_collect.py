...
class TestQualityScoring:
    def _make_skill(self):
        with patch("toolloop_arb_collect.GeckoTerminalClient"), \
            ...
            return ArbCollectSkill(config={"output_root": "/tmp/test_collect"})
...
class TestDataclasses:
    ...
    def test_collect_outcome_defaults(self):
        o = CollectOutcome()
...
def _make_mock_prompts(available: set[str] | None = None) -> MagicMock:
    """创建 PromptStore mock，所有 collect_ 开头的 prompt 都可用"""
    if available is None:
        available = {
            "collect_flash_classify_system",
            "collect_flash_classify_user",
            "collect_pro_arbitrate_system",
