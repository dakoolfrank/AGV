...
| 注入点 | AGV 实现 | 说明 |
|--------|---------|------|
| `PipelineProfile` | `AGV_TRUNK_PROFILE`（5 步主干）+ `S5_MM_PROFILE` + `S5_ARB_PROFILE` | 主干 S1→S2→S3→S4 + S5 双分支 |
| `CampaignConfig` | `RWACampaignConfig` / `MMCampaignConfig` / `ArbCampaignConfig` | 回退策略表 + 终止条件（分支独立预算） |
| `DiagnosisProfile` | `ChainDiagnosisProfile` | Solidity/链上错误诊断 prompt |
| `ToolExecutor` | `ForgeExecutor` / `VercelExecutor` / `DexExecutor` | 工具执行后端 |

## §3 流水线总览（主干 + S5 分支）

> **v4.2 拓扑演进**：S2 产出链上状态后**分叉**——主干走 S3→S4（数据→展示→传播），S5 分支走 MM/Arb（流动性管理）。
> 两条分支共享 S2 的 `lp_state` / `token_state`，但互不阻塞。
> S3 内部拆为 L1 (Web3) / L2 (Web2)，与 WQ-YI 的 `dataset_l1` / `field_updater` / `dataset_l2` 拆分逻辑一致。

### 拓扑图

```
         S1 (Oracle) → S2 (Chain Ops) ─┬─→ S3.L1 (Web3) → S3.L2 (Web2) → S4 (KOL)
                                       │   主干：数据 → 展示 → 传播
                                       │
                                       └─→ S5 MM-Campaign / S5 Arb-Campaign
                                           分支：流动性管理（独立 CampaignRunner）
```

**分叉点 = S2 完成后**：S3.L1 和 S5 都消费 S2 的 `token_state` / `nft_state` / `lp_state`，但 S5 不消费 S3 产出、S3 也不消费 S5 产出。

### 步骤定义

```python
# ── 主干（Orchestrator 编排）──
AGV_TRUNK_STEP_ORDER = [
    "asset_oracle",    # S1
    "chain_ops",       # S2 ← 分叉点
...
# ── S5 分支（独立 CampaignRunner，不在 Orchestrator step_order 中）──
S5_MM_STEPS = ["monitor", "detect", "decide", "execute", "log"]
S5_ARB_STEPS = ["scan", "curate", "dataset", "execute", "fix"]
...
AGV_STEP_TO_SUBAGENT = {
    ...
    "kol":            "agv-kol",             # S4
    # S5 分支（共用一个 Subagent，双 Campaign）
    "monitor":  "agv-market-maker",  # S5-MM
    "detect":   "agv-market-maker",
    "decide":   "agv-market-maker",
    "scan":     "agv-market-maker",  # S5-Arb
    "curate":   "agv-market-maker",
    "dataset":  "agv-market-maker",
    "execute":  "agv-market-maker",  # S5 共享
...
| **S5-MM monitor** | `pool_state` | `lp_state` *(从 S2 分叉消费)* |
| **S5-MM execute** | `heartbeat_log`, `tx_result` | `pool_state`, `anomaly_signal` |
| **S5-Arb scan** | `market_signal` | `lp_state` *(从 S2 分叉消费)* |
| **S5-Arb execute** | `execution_result` | `arb_strategy` |

### S5 双 Campaign 共享基础设施

MM-Campaign 和 Arb-Campaign 虽是独立 CampaignRunner，但共享以下基础设施：

| 共享层 | 内容 | 独占层 |
|--------|------|--------|
| **DexExecutor L2** | `swap()/get_reserves()/add_liquidity()` 统一接口 | 各自的 PipelineDescriptor |
| **三大安全护甲** | SlippageGuard + MEVGuard + TVLCircuitBreaker | 各自的 CampaignConfig |
| **ExecutorConfig** | 硬编码安全底线（$50单笔/2%滑点/$500日量） | 各自的 DiagnosisConfig |
| **ApproveManager** | Token approve 策略（需求×2，禁止 MAX_UINT256） | 各自的 toolloop 入口 |
| **nexrur P0-P3** | outcome/audit/evidence/gate 治理 | 各自的 LLM 角色 |
| **knowledge/ + SKILL.md** | 4 个 YAML + 8 个 prompt 模板 | — |
