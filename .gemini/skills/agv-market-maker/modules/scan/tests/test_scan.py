...
from toolloop_scan import SignalBus, ScanLoop
...
class TestScanSkill:
    ...
    def test_known_tokens(self):
        assert "0x5558e43eE316C45e6C842bC7aC4B770EED03c5C0" in ScanSkill.KNOWN_TOKENS
        assert "0xBE1B08D1743f2C158165472Fa2fEB038E8DfaA9d" in ScanSkill.KNOWN_TOKENS
    ...
    def test_run_delegates_to_fusion(self):
        """ScanSkill.run delegates to DataFusion.detect_signals"""
    ...
    def test_scan_all_pools_aggregates(self):
        """scan_all_pools collects signals from multiple pools"""
        mock_fusion = DataFusion()
        mock_fusion.detect_signals = AsyncMock(return_value=[
        ...
        result = _run(skill.scan_all_pools(["0xAAA", "0xBBB"]))
...
class TestScanLoop:
    ...
    def test_init_custom_interval(self):
        loop = ScanLoop(interval_seconds=30.0)
    ...
    def test_run_once_no_skill_raises(self):
        ...
        try:
            ...
            assert "scan_skill not configured" in str(e)

    def test_degraded_interval(self):
        loop = ScanLoop(interval_seconds=60.0, degraded_interval=300.0,
    ...
    def test_run_once_increments_cycle(self):
        ...
        mock_skill.scan_all_pools = AsyncMock(return_value=[])
        loop = ScanLoop(scan_skill=mock_skill)
    ...
    def test_run_once_resets_noop_on_signals(self):
        ...
        bus = SignalBus()
        loop = ScanLoop(scan_skill=mock_skill, signal_bus=bus)
        loop._noop_count = 10
        _run(loop.run_once(["0xABC"]))
