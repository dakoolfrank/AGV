...
async def run_mm_campaign(*, config: dict | None = None, workspace: Path | None = None):
    ...
    adapter = PancakeV2Adapter(web3=cfg.get("web3"))
    executor = DexExecutor(adapter=adapter, signer=cfg.get("signer"))
...
async def run_arb_campaign(*, config: dict | None = None, workspace: Path | None = None):
    ...
    from toolloop_mm import DexExecutor, PancakeV2Adapter, NotifyRouter
