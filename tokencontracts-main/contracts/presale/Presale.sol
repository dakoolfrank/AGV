...
contract Presale is AccessControl, ReentrancyGuard, Pausable {
    ...
    struct SaleStage {
        uint256 price;         // USDT per preGVT (6-decimal USDT for 1e18 preGVT)
    ...
    constructor(
        ...
        require(_preGVT != address(0), "Invalid preGVT");
     ...
     * @param recipient Address to receive preGVT (can differ from msg.sender for agent sales)
    ...
    function buy(
        ...
        // price is in 6-decimal USDT for 1e18 preGVT
        // Example: price = 5_000 means 0.005 USDT per 1 preGVT
        //   usdtAmount = 1_000_000 (1 USDT) → (1_000_000 * 1e18) / 5_000 = 200e18 preGVT
        uint256 preGVTAmount = (usdtAmount * 1e18) / stage.price;
        if (preGVTAmount == 0) revert AmountTooSmall();

        // Stage cap check
        if (stage.sold + preGVTAmount > stage.cap) revert StageCapExceeded();

        // Per-address limit check
        if (stage.maxPerAddress > 0) {
            if (purchased[currentStageId][recipient] + preGVTAmount > stage.maxPerAddress) {
     ...
     * @param price USDT per preGVT (6-decimal USDT for 1e18 preGVT). e.g. 5000 = 0.005 USDT
     * @param cap Max preGVT sellable (18 decimals)
     * @param startTime Unix timestamp for sale open
     * @param endTime Unix timestamp for sale close
     * @param maxPerAddress Max preGVT per wallet (0 = unlimited)
    ...
    /**
     * @notice Remaining preGVT available in current stage
     */
